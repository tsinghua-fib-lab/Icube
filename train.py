...
def syn_community1v2(args, writer=None, export_graphs=False):
    ...
    graphs = []
    with open('../prediction_in_full_graph_with_aoi/GMNN_data/node_label.json', 'r') as f:
        graph_label = json.load(f)
    for i in tqdm(os.listdir('../prediction_in_full_graph_with_aoi/GMNN_data/train_check')):
        case_num = extract_numbers(i)
        G = nx.Graph()
        with open('../prediction_in_full_graph_with_aoi/GMNN_data/net.txt', 'r') as f:
            for line in f:
                start, end, _ = line.strip().split("\t")
                G.add_edge(start, end)
        with open('../prediction_in_full_graph_with_aoi/GMNN_data/train/{0}'.format(i), 'r') as file:
            content = file.read()
        nodes = content.split("\n")
        feat_dict = {}
        f_feature = np.load('../prediction_in_full_graph_with_aoi/GMNN_data/path/{0}.npy'.format(case_num))
        for index, feature in enumerate(f_feature):
            node_feature = []
            node_feature.append(1 if index in nodes else 0)
            node_feature.append(feature)
            feat_dict[str(index)] = {'feat': np.array(node_feature)}
        nx.set_node_attributes(G, feat_dict)
        G.graph['label'] = graph_label[case_num]
        G.graph['case_num'] = case_num
        graphs.append(G)
